...
##Filter rows where confidence is A, B or C
net = dorothea_hs %>%
...
#Join to find if the target gene is also a DEI in the same cell type
TF_target_DEI_pairs <- regulon_with_DE_TFs %>%
...
DEITFs$cell_type <- factor(DEITFs$cell_type, levels = desired_order)
...
global_log2fc_vals <- TF_target_DEI_pairs %>%
...
plot_tf_network <- function(cell_type_name) {
  network_df <- TF_target_DEI_pairs %>%
...
TF_target_DEI_pairs <- regulon_with_DE_TFs %>%
  inner_join(target_DEIs, by = c("target" = "gene", "TF_cell_type" = "cell_type")) %>%
  rename(target_log2FC = log2FoldChange,
         target_padj = padj,
